import { BaseEntity, VikaOptions, MappingOptions } from '../src/utils/vika-orm' // 导入 BaseEntity, VikaOptions, 和 MappingOptions 类型/类
...
console.debug('保存newUser1:', JSON.stringify(res))  // 输出新用户的信息
...
console.debug('保存newUser2:', JSON.stringify(newUser2))  // 输出新用户的信息
await wait(500)  // 等待 500 毫秒

// 改
const updateRes = await User.update(newUser2.recordId, { email: 'newalice@example.com' } as Partial<User>)  // 更新新用户的电子邮件
console.debug('更新newUser2:', JSON.stringify(updateRes))  // 输出更新后的新用户信息
await wait(500)  // 等待 500 毫秒

// 查
const queryRes = await User.findById(newUser2.recordId)
console.debug('查询findById:', JSON.stringify(queryRes))  // 输出更新后的新用户信息
await wait(500)  // 等待 500 毫秒

const query2Res = await User.findByField('email', 'bob@example.com')
console.debug('查询findByField:', JSON.stringify(query2Res))  // 输出更新后的新用户信息
await wait(500)  // 等待 500 毫秒

// 删
const deleteRes = await User.delete(newUser2.recordId)  // 删除新用户
console.debug('删除newUser2:', JSON.stringify(deleteRes))  // 输出更新后的新用户信息
await wait(500)  // 等待 500 毫秒

// 查
const users = await User.findAll()  // 查找所有用户（当前被注释掉）
console.debug('查询users:', JSON.stringify(users))  // 输出新用户的信息
await wait(500)  // 等待 500 毫秒
}
